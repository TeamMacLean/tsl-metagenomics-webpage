...
# Metagenome Binning {#sec-05a-metagenome-binning}
...
![Overview of MAG binning](fig/05a-01.png)

Although an obvious way to separate contigs that correspond to a different species is by their taxonomic assignation, there are more reliable methods that do the binning using 
...
+ A summary file with information on each genome bin, including its length, completeness, contamination, and taxonomy classification (.txt)
...
Using the information contained in the summary file:

1) How many bins does your sample have?
...
1) There are X bins for sample 98
2) X sequences are classified in the second bin.

::

ADD MOIRE HERE REGARDING THE PRECOMPUTED DATA
